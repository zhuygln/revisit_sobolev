...
def run_fontes(state, shells, n, legs=LEGS, seeds=L.SEEDS, tau_min=1e-3, emis_cut=1e-6, f_min=1e-3,
               budget_s=None, verbose=True, dataset=None, core="reemit", core_max_passes=200):
    t0 = time.time()
    ...
    row = dict(state=state.meta["name"], t_d=state.t / DAY, shells=list(map(int, shells)), n_shell=len(shells),
               ...
               core_max_passes=core_max_passes,
    ...
    for tag in legs:
        ...
        res = [run_mc(atom, zone["r_core"], zone["r_out"], zone["t_exp"], lo, hi, n, spec["mode"], seed=s,
                      ...
                      launch_weight="energy", launch="volume", launch_core_frac=core_frac, core=core, core_max_passes=core_max_passes, wall_s=budget_s) for s in seeds]
...
def main():
    ...
    ap.add_argument("--max-passes", type=int, default=200, help="core returns before a packet is booked to the core (a large value with 'reflect' = lossless steady state)")
