...
merged (main carries the fixed transport). **Phase 10b (F65, §4.60) is done:** the Fontes benchmark as a time-slab light curve gives the same bolometric ordering under both redistributions (line-binned within 2–4 %, expansion +15–17 %) and colour errors that fluorescence raises from 0.13 to 0.51 mag (line-binned; the high-cap rerun changed nothing but the cap fraction, 1.4 → 1.2 %, so R2 stays Gray by rule) and 0.20 to 0.32 (expansion). **Phase 10c (F66, §4.61) is done:** the P1 composition on the published xkn secular structure, 2–8 d, with the xkn diffusion solution as the photospheric boundary and the outer ejecta transported with explicit fluorescence — no statistically resolved bolometric ordering (peaks within 1.4 %, inside the 1.6 % noise; R2 Gray by rule) but distinct chromatic biases, expansion a 0.3 mag blueward offset in i − K, line-binned a 0.4 mag redward one (the review pass replaced the lab-frame re-emitting photosphere by a comoving-frame one, `core reemit_cm`; the closures moved by ≤ 0.09 mag). The manuscript draft is in `docs/paper4/`. `make check` verifies the freeze, the structure and the macro usage; `make claims` is the pre-submission read-through of every sentence that quotes a number, with the macro replaced by the frozen value (the PI's standing rule after 2026-09-11: no number in a table or a figure annotation is transcribed by hand). **Scope frozen (PI, 2026-09-11, verbatim in `plan_review.md`):** no deeper P1 transport, no gas-energy equilibrium, no NLTE, no Phases 9–10, no further abundance or parameter grids before submission; the review pass has run (an independent accounting/photosphere/provenance audit, which found the lab-frame re-emitting boundary, plus the claim read-through) and PR #3 is merged; what remains is a referee-style human read of the manuscript, then wording and presentation only. Superseded by that freeze: Phases 9–10 (estimators, full
