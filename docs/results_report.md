...
**Supplementary Information and cover letter (2026-09-03, later).**
`docs/paper3/si.tex` → `si.pdf` (6 pp) carries the tables the display items
summarize, each generated by `latex_tables.py` from FROZEN: S1 Gate 2 for
every leg under every rule and tangent space (7 × 4 rows: baseline,
floored-included, absorbing core, chain cap 8000, T1, T2, T3), S2 leg C at
every grid point under T0–T3 (27 rows, sideways), S3 the chain-cap test per
cell and cap (4 × 3 rows), S4 the Monte Carlo temperature derivatives
against the Planck proxy per live observable at the central point, S5 the
closure error per band–epoch key (38 rows: n, median, 16–84 %, C_bin
median, mode shape). The SI prose names the two harness conventions
(floored epochs; absorbing core, which moves three points to C-A because
the grey normalization term differs between the reference and the closure
legs — a convention T1 subsumes by construction) and states the reason the
chain-cap δm_ref and δd_RT columns agree to rounding (the closure legs do
not chain). `check_structure.py` now runs the literal-number ban, the
retracted-value ban, the `\input`-fragment and `\todo` checks on `si.tex`
too (`check_si()`); `make` builds both PDFs. `_f` no longer prints "−0.00".
`cover_letter.md` follows Paper I's template: the six beats, why the message
is broader than kilonova codes, the three expectation-setting points
(differential claim; restricted experiment with named future work; the
failed half of the chain-cap criterion reported with its uncertainty) and
the frozen-number build. Tests: `test_si_tables_have_the_expected_rows`,
...
### 4.62 Paper B gate G1: does the compression survive correct energy physics? (F67)
...
3×10⁵ energy packets per seed (Nd II: 10⁶, the preregistered gray remedy,
see below). Records `paperB/gate1/gate1_<ion>.json`,
...
Nd II at 3×10⁵ was **Gray on condition 2**: g carries ≥ 1 % of the
luminosity but its R2 seed scatter was 0.063 > 0.05 mag. The preregistered
remedy — the whole Nd grid at a higher packet count, never one leg — was
run at 10⁶ packets per seed (2.9 h; `gate1_60NdII_n1e6.json`; the 3×10⁵
record is kept): g's scatter fell to 0.005, all seven bands are live,
nothing is gray. K128 in/out 0.005; 8.2 events per packet:

| N_g | N_g² | exit samples | kB | band max | band mean | SED | event | colour |
|---|---|---|---|---|---|---|---|---|
| 2 | 4 | 86,187 | 2023 | 0.086 | 0.019 | 0.015 | 0.489 | 0.065 |
| 4 | 16 | 86,187 | 2024 | 0.054 | 0.014 | 0.013 | 0.404 | 0.052 |
| 8 | 64 | 86,187 | 2025 | 0.022 | 0.011 | 0.012 | 0.256 | 0.026 |
| 16 | 256 | 86,187 | 2029 | 0.013 | 0.007 | 0.012 | 0.196 | 0.018 |
| 32 | 1024 | 86,187 | 2044 | 0.023 | 0.008 | 0.012 | 0.161 | 0.017 |
| ε* = 0.00 | 1 | — | — | 1.058 | 0.499 | 0.562 | — | 1.054 |

**Readings (as preregistered).** N_g* is the smallest N_g with band max
≤ 0.10 and colour max ≤ 0.10: **La II 2, Ce II 16, Nd II 2**. B1 **Green**:
two ions reach the threshold by 8 groups (both by 2) and the third by 32.
B2: Ce II **Green** (ε* = 0, pure resonant scattering, is the best scalar
and it is 1.40 mag off, 23 times the 16-group matrix's 0.060); Nd II
**Green** (ε* = 0 at 1.06 mag against the 2-group matrix's 0.086); La II
**Yellow** (ε* = 0.20 is 0.068 mag off, under the 0.20 mag "substantial"
floor and 1.9 times the 2-group matrix's 0.036: on this thin ion nothing is
far from anything); two Greens → **Green**. B3 **Yellow**: every metric
converges from 8 to 32 groups on every ion and the event metric is
monotone everywhere, but the letter of the rule counts an 0.001 wiggle in
Ce II's SED error (16 → 32) and Nd II's band error rising 0.013 → 0.023 at
32 groups, at the edge of its 0.01 mag noise. **Decision: CONTINUE toward
the PRL** — B1 Green, B2 Green, B3 not Red — and the next step is G2, why.
...
under photon packets: Nd II compresses to two groups (0.086 mag; 0.022 at
eight), Ce II needs sixteen for 0.1 mag and thirty-two for 0.025. (iii) The serialized kernel is not
N_g² numbers: the discrete exit tables dominate it (Ce 12,510 samples,
297 kB at every N_g; Nd 86,187 samples at 10⁶ packets, 2.0 MB — the table
grows with the event corpus, the matrix does not), and the matrix adds
1–8 %.
...
at 32 groups, against a sampling floor of 0.005–0.024) while the
observables are already converged: most of the
...
### 4.63 Paper B after G1: the compression under the radiation-field-driven macroatom, and what the exit tables hold (F68)

Two of the PI's three post-G1 steps (`paperB/plan_review.md`, 2026-09-22),
both run on 2026-09-22; the third, G2, is preregistered in
`paperB/prl_gate.md` and not run.

**(a) The R2M robustness check** (the post-pass paragraph of G1, fixed as
its own section of `prl_gate.md` before the run; not a gate). Reference:
`R2M`, Lucy's macroatom with internal upward transitions under an imposed
W = ½ B_ν(T_zone) (`sobolev_macro`, F63's instrument), on the evaluation
seeds 1–3; the 8-group operator `A2M_ng8` rebuilt from `R2Mbuild`'s events
on the build seeds 101–103; G1's downward-trained `A2_ng8` and `R2`
scored against R2M in the same run, seed for seed; 3×10⁵ energy packets
per seed for all three ions (the section states the memory bound that
kept Nd II off 10⁶; no precision gray fired: R2M's seed scatter is
0.008 / 0.016 / 0.025 mag at most, below R2's). Records
`paperB/r2m/r2m_<ion>.json`, `r2m_verdict.json`; the tables are generated
by `paperB/r2m/analyse_r2m.py --markdown`.

| ion | live bands | R2M − R2 (g r i z J H K) | max shift | events/packet R2M vs R2 | E_esc / E_core / W (R2M) | survives |
|---|---|---|---|---|---|---|
| La II | g r i z J H K | -0.07 +0.06 +0.05 +0.01 -0.01 -0.00 -0.00 | 0.07 | 0.8 vs 0.7 | 0.852 / 0.127 / 0.021 | **YES** |
| Ce II | g r i z J H K | -2.09 -1.88 -1.72 -1.58 -1.18 -0.75 +0.53 | 2.09 | 13.7 vs 5.4 | 0.247 / 0.644 / 0.109 | **YES** |
| Nd II | g r i z J H K | -0.89 -0.87 -0.79 -0.58 -0.79 -0.39 +0.10 | 0.89 | 13.4 vs 8.2 | 0.290 / 0.608 / 0.102 | **YES** |

| ion | operator (scored against R2M) | max abs dm | mean abs dm | max abs dcolour | SED L1 | m_event vs K128M | exit lines | kB |
|---|---|---|---|---|---|---|---|---|
| La II | R_8 from R2M's events | 0.015 | 0.005 | 0.027 | 0.005 | 0.515 | 541 | 18 |
| La II | R_8 from R2's events (downward-trained) | 0.056 | 0.026 | 0.105 | 0.019 | 0.516 | 524 | 17 |
| La II | R2 itself (the downward macroatom) | 0.069 | 0.029 | 0.126 | 0.019 | — | — | — |
| La II | K128Mbuild vs K128M (sampling floor) | | | | | 0.023 | 557 | 292 |
| Ce II | R_8 from R2M's events | 0.026 | 0.012 | 0.025 | 0.026 | 0.343 | 13777 | 328 |
| Ce II | R_8 from R2's events (downward-trained) | 1.965 | 1.339 | 1.700 | 0.704 | 0.389 | 12510 | 298 |
| Ce II | R2 itself (the downward macroatom) | 2.094 | 1.390 | 1.707 | 0.705 | — | — | — |
| Ce II | K128Mbuild vs K128M (sampling floor) | | | | | 0.005 | 13809 | 603 |
| Nd II | R_8 from R2M's events | 0.046 | 0.016 | 0.027 | 0.025 | 0.284 | 70661 | 1661 |
| Nd II | R_8 from R2's events (downward-trained) | 0.939 | 0.630 | 0.846 | 0.465 | 0.295 | 59270 | 1394 |
| Nd II | R2 itself (the downward macroatom) | 0.886 | 0.628 | 0.889 | 0.465 | — | — | — |
| Nd II | K128Mbuild vs K128M (sampling floor) | | | | | 0.006 | 70481 | 1931 |

Reading. **The compression survives the radiation-field-driven macroatom
for all three ions**: the 8-group operator rebuilt from R2M's events
reproduces R2M to 0.015 / 0.026 / 0.046 mag (max over the seven live
bands), colours to ≤ 0.027 mag, the SED to ≤ 0.026 — at least as close
as G1's 8-group operator came to the downward reference (0.013 / 0.122 /
0.022 mag). The reference itself moves as F63 found on the blend: Ce II
brightens by 1.6–2.1 mag in g–z and dims by 0.5 in K, Nd II by 0.6–0.9
mag with K +0.1, La II by ≤ 0.07 (0.8 events per packet). The
downward-trained operator scored against R2M reproduces *R2*, not R2M
(Ce II 1.97 mag off R2M against R2's own 2.09; Nd II 0.94 against 0.89):
the operator is an effective closure of the fluorescence physics it was
built from, and carries that physics faithfully to the light — it is not
a physics-independent law. The event-level loss at 8 groups against
R2M's independent fine matrix is 0.52 / 0.34 / 0.28 with sampling floors
0.023 / 0.005 / 0.006, the same order as G1's; the R2M kernels store more
exit lines than the downward ones (Ce II 13,777 against 12,510; Nd II
70,661 against 59,270 at the same packet count): the upward jumps open
exit channels the downward cascade never reaches.

**(b) The exit-table audit** (representation only; `paperB/audit/exit_tables.py`,
record `exit_tables.json`, figure `docs/figures/paperB/exit_table_audit.png`;
the table is generated by `--markdown`). Checked in the code before any
number was run: `RedistributionKernel.from_branching_mc` aggregates the
exits by unique frequency with the energy weights summed, so the
proposed exact aggregation is already the implementation, and its
premise — repeated events stored — does not hold. The audit reruns
`R2build` at each ion's G1 packet count and measures what the tables
hold.

| ion | packets × seeds | events | lines in the list | opacity lines | distinct exit lines | exact rest frequencies | exit energy in opacity lines | lines carrying 50 / 90 / 99 / 99.9 % | kept per group at f = 0.1 / 0.2 / 0.5 / 0.9 / 0.99 / 0.999 | K128 kB: total = matrix + tables | G1 record | K at N_g*: kB (tables) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
80| La II | 300,000 × 3 | 671,241 | 17,743 | 2,004 | 524 | 1.000 | 1.000 | 35 / 138 / 249 / 343 |  / 92| La II | 300,000 × 3 | 671,241 | 17,743 | 2,004 | 524 | 1.000 | 1.000 | 35 / 138 / 249 / 343 |  / 146| La II | 300,000 × 3 | 671,241 | 17,743 | 2,004 | 524 | 1.000 | 1.000 | 35 / 138 / 249 / 343 |  / 264| La II | 300,000 × 3 | 671,241 | 17,743 | 2,004 | 524 | 1.000 | 1.000 | 35 / 138 / 249 / 343 |  / 347| La II | 300,000 × 3 | 671,241 | 17,743 | 2,004 | 524 | 1.000 | 1.000 | 35 / 138 / 249 / 343 |  / 427 | 291 = 277 + 14 | 291 kB, 524 | N_g* = 2: 16 (13) |
285| Ce II | 300,000 × 3 | 4,884,220 | 408,639 | 59,918 | 12,510 | 1.000 | 0.993 | 349 / 2,356 / 6,495 / 9,981 |  / 535| Ce II | 300,000 × 3 | 4,884,220 | 408,639 | 59,918 | 12,510 | 1.000 | 0.993 | 349 / 2,356 / 6,495 / 9,981 |  / 1,482| Ce II | 300,000 × 3 | 4,884,220 | 408,639 | 59,918 | 12,510 | 1.000 | 0.993 | 349 / 2,356 / 6,495 / 9,981 |  / 4,474| Ce II | 300,000 × 3 | 4,884,220 | 408,639 | 59,918 | 12,510 | 1.000 | 0.993 | 349 / 2,356 / 6,495 / 9,981 |  / 8,314| Ce II | 300,000 × 3 | 4,884,220 | 408,639 | 59,918 | 12,510 | 1.000 | 0.993 | 349 / 2,356 / 6,495 / 9,981 |  / 10,873 | 572 = 277 + 295 | 572 kB, 12,510 | N_g* = 16: 303 (294) |
366| Nd II | 1,000,000 × 3 | 24,638,694 | 3,336,077 | 88,611 | 86,187 | 1.000 | 0.976 | 476 / 4,061 / 22,922 / 61,529 |  / 697| Nd II | 1,000,000 × 3 | 24,638,694 | 3,336,077 | 88,611 | 86,187 | 1.000 | 0.976 | 476 / 4,061 / 22,922 / 61,529 |  / 2,027| Nd II | 1,000,000 × 3 | 24,638,694 | 3,336,077 | 88,611 | 86,187 | 1.000 | 0.976 | 476 / 4,061 / 22,922 / 61,529 |  / 9,742| Nd II | 1,000,000 × 3 | 24,638,694 | 3,336,077 | 88,611 | 86,187 | 1.000 | 0.976 | 476 / 4,061 / 22,922 / 61,529 |  / 33,822| Nd II | 1,000,000 × 3 | 24,638,694 | 3,336,077 | 88,611 | 86,187 | 1.000 | 0.976 | 476 / 4,061 / 22,922 / 61,529 |  / 66,045 | 2299 = 277 + 2022 | 2299 kB, 86,187 | N_g* = 2: 2023 (2021) |

| ion | distinct exit lines after 10⁴ / 10⁵ / 10⁶ / all events |
|---|---|
| La II | 306 / 423 / 524 / 524 (of 671,241 events) |
| Ce II | 2,723 / 6,616 / 10,584 / 12,510 (of 4,884,220 events) |
| Nd II | 3,439 / 11,336 / 31,271 / 86,187 (of 24,638,694 events) |

Reading. Every stored exit is an exact line rest frequency (fraction
1.000 for all three ions); the stored count is the number of *distinct
exit lines the sample discovered*, 3.0 / 3.1 / 2.6 % of the line list.
The audit's 128-group kernels reproduce G1's recorded `K128build` sizes
to the byte (291 / 572 / 2,299 kB: the same seeds, the same events). The
128-group matrix part is 277 kB for every ion (R, the photon rows and the
sub-histograms at 128²); the tables are 24 bytes per exit line — 14 /
295 / 2,022 kB — and at N_g* the matrix is 2–8 kB while the tables are
unchanged. La II's table saturates (524 lines from 10⁶ events on, of
2,004 opacity lines), Ce II's nearly (10,584 at 10⁶ events, 12,510 at
4.9×10⁶), Nd II's does not (31,271 at 10⁶, 86,187 at 2.5×10⁷ and still
rising): rare exit lines keep being discovered. The energy is
concentrated: 99 % of the exit energy in 249 / 6,495 / 22,922 lines
(48 / 52 / 27 % of the distinct lines) and 90 % in 138 / 2,356 / 4,061
(26 / 19 / 4.7 %); 97.6–100 % of it lands in lines that are themselves
opacity lines (τ ≥ 10⁻³), i.e. re-absorbable. A per-output-group
truncation keeping fraction f of each group's exit energy — the operator
G2's H2 transports — keeps La II 80 / 92 / 146 / 264 / 347 / 427, Ce II 285 / 535 / 1,482 / 4,474 / 8,314 / 10,873 and Nd II 366 / 697 / 2,027 / 9,742 / 33,822 / 66,045 lines at f = 0.1 / 0.2 / 0.5 / 0.9 / 0.99 / 0.999 (Ce II 2.3 / 4.3 / 12 / 36 / 67 / 87 % and Nd II 0.4 / 0.8 / 2.4 / 11 / 39 / 77 % of their distinct lines), which set G2's f grid: H2's Green (the light needing ≤ 10 % of the lines) is reachable at f ≤ 0.2 on Ce II and f ≤ 0.5 on Nd II.

**F68 — Under the radiation-field-driven macroatom (imposed W = ½ field,
upward transitions, the reference moving by up to 2.1 mag) the 8-group
operator rebuilt from that reference's events reproduces it to
0.015–0.046 mag on all three ions, while the downward-trained operator
reproduces the downward reference it came from; the compression G1
found is not a property of the downward table. The kernel's exit tables
are already the exact aggregate over distinct exit lines (every entry an
exact rest frequency, 24 bytes each); their size is set by how many exit
lines the sample discovers — saturating for La II and Ce II, still
rising at 2.5×10⁷ events for Nd II — not by the event count, and 90 % of
the exit energy sits in 5–26 % of them.** Whether the light needs the
rest is G2's H2, an experiment, not a representation fix.

### 4.64 Paper B gate G2: locality or low rank, and how much of the exit spectrum the light needs (F69)

Preregistered in `paperB/prl_gate.md` "G2", approved by the PI with the
H1 amendment (`paperB/plan_review.md`, 2026-09-22) and frozen at the tag
`paperB-g2-prereg`; run 2026-09-22. Three operator families, each a
128-group energy matrix on G1's edges carrying **the same** 128-group
discrete exit tables built from `R2build`, each transported through
`sobolev_group` with energy rows on the evaluation seeds and scored
against G1's reference `R2` with G1's live-band rule, metrics and
thresholds (max |Δm| ≤ 0.10 mag and max |Δcolour| ≤ 0.10 mag). The
families differ only in what they do to the group-to-group matrix, or for
the truncation family to the tables. Packets as in G1: La II and Ce II
3×10⁵ per seed, Nd II 10⁶. Records `paperB/gate2/gate2_<ion>.json`,
`gate2_verdict.json`; figure
`docs/figures/paperB/gate2_locality_vs_rank.png`; tables generated by
`paperB/gate2/analyse.py --markdown`.

The matched axis is the **number of archetypal exit distributions**: a
local operator at N_g offers N_g of them, chosen by which contiguous
frequency block the absorbed frequency falls in; a global rank-k operator
offers k, mixed freely with weights fitted per fine input group. At equal
count the global family is far the more expressive — 2·128·k free numbers
against N_g² — so needing no fewer archetypes than the local family is
the evidence for locality. Two amendments were made to the
preregistration during execution and are recorded there in full: two
implementation defects in computing the factorisation (a live row sent to
zero, and 600 iterations that did not converge), and the firing of the
`L128_ng8` control on both decisive ions, whose declared remedy — the
local family transported on the 128-group tables rather than read from
G1's coarse legs — was applied without touching any threshold.

La II (control), live bands g r i z J H K; the local control differs from the coarse kernel by 0.4 σ; K*_local = 2, K*_global = 2, L* = 0.504; **H1 GREEN, H2 RED**:

| family | archetypes | matrix parameters | max abs dm | mean abs dm | max abs dcolour | SED L1 | m_event | exit lines kept | fallback |
|---|---|---|---|---|---|---|---|---|---|
| local, N_g = 2 | 2 | 4 | 0.014 | 0.003 | 0.014 | 0.006 | 0.431 | all | — |
| local, N_g = 4 | 4 | 16 | 0.019 | 0.003 | 0.018 | 0.006 | 0.429 | all | — |
| local, N_g = 8 | 8 | 64 | 0.006 | 0.001 | 0.007 | 0.005 | 0.423 | all | — |
| local, N_g = 16 | 16 | 256 | 0.007 | 0.002 | 0.004 | 0.005 | 0.343 | all | — |
| local, N_g = 32 | 32 | 1024 | 0.004 | 0.002 | 0.006 | 0.006 | 0.311 | all | — |
| global, rank 1 | 1 | 256 | 0.972 | 0.333 | 0.526 | 0.246 | 0.702 | all | 0.0007 |
| global, rank 2 | 2 | 512 | 0.069 | 0.023 | 0.072 | 0.031 | 0.505 | all | 0.0001 |
| global, rank 4 | 4 | 1,024 | 0.172 | 0.038 | 0.127 | 0.034 | 0.467 | all | 0.0001 |
| global, rank 8 | 8 | 2,048 | 0.031 | 0.008 | 0.031 | 0.021 | 0.356 | all | 0.0000 |
| global, rank 16 | 16 | 4,096 | 0.054 | 0.010 | 0.052 | 0.015 | 0.241 | all | 0.0000 |
| global, rank 32 | 32 | 8,192 | 0.034 | 0.009 | 0.017 | 0.010 | 0.093 | all | 0.0000 |
| truncation, f = 0.1 | 128 | 16,384 | 0.317 | 0.077 | 0.451 | 0.041 | 0.024 | 80 (0.153) | 0.0000 |
| truncation, f = 0.2 | 128 | 16,384 | 0.320 | 0.076 | 0.446 | 0.039 | 0.024 | 92 (0.176) | 0.0000 |
| truncation, f = 0.5 | 128 | 16,384 | 0.073 | 0.021 | 0.124 | 0.017 | 0.024 | 146 (0.279) | 0.0000 |
| truncation, f = 0.9 | 128 | 16,384 | 0.015 | 0.003 | 0.020 | 0.006 | 0.024 | 264 (0.504) | 0.0000 |
| truncation, f = 0.99 | 128 | 16,384 | 0.021 | 0.004 | 0.022 | 0.005 | 0.024 | 347 (0.662) | 0.0000 |
| truncation, f = 0.999 | 128 | 16,384 | 0.014 | 0.003 | 0.011 | 0.005 | 0.024 | 427 (0.815) | 0.0000 |

Diagnostic at the matched count k = 2: the global operator's event-level loss is 0.505 against the local operator's 0.431, and its band error 0.069 against 0.014 — `events_vs_observables` **false**.

Ce II, live bands r i z J H K; the local control differs from the coarse kernel by 2.6 σ; K*_local = 16, K*_global = 32, L* = 0.358; **H1 GREEN, H2 YELLOW**:

| family | archetypes | matrix parameters | max abs dm | mean abs dm | max abs dcolour | SED L1 | m_event | exit lines kept | fallback |
|---|---|---|---|---|---|---|---|---|---|
| local, N_g = 2 | 2 | 4 | 0.265 | 0.116 | 0.201 | 0.041 | 0.289 | all | — |
| local, N_g = 4 | 4 | 16 | 0.146 | 0.074 | 0.119 | 0.037 | 0.244 | all | — |
| local, N_g = 8 | 8 | 64 | 0.106 | 0.063 | 0.115 | 0.038 | 0.238 | all | — |
| local, N_g = 16 | 16 | 256 | 0.063 | 0.033 | 0.067 | 0.030 | 0.182 | all | — |
| local, N_g = 32 | 32 | 1024 | 0.032 | 0.018 | 0.051 | 0.027 | 0.138 | all | — |
| global, rank 1 | 1 | 256 | 1.023 | 0.605 | 0.617 | 0.336 | 0.389 | all | 0.0000 |
| global, rank 2 | 2 | 512 | 0.994 | 0.579 | 0.626 | 0.270 | 0.385 | all | 0.0000 |
| global, rank 4 | 4 | 1,024 | 0.454 | 0.243 | 0.564 | 0.144 | 0.265 | all | 0.0000 |
| global, rank 8 | 8 | 2,048 | 0.258 | 0.152 | 0.171 | 0.124 | 0.214 | all | 0.0000 |
| global, rank 16 | 16 | 4,096 | 0.148 | 0.106 | 0.093 | 0.067 | 0.130 | all | 0.0000 |
| global, rank 32 | 32 | 8,192 | 0.081 | 0.047 | 0.034 | 0.033 | 0.058 | all | 0.0000 |
| truncation, f = 0.1 | 128 | 16,384 | 0.209 | 0.140 | 0.387 | 0.305 | 0.010 | 285 (0.023) | 0.0000 |
| truncation, f = 0.2 | 128 | 16,384 | 0.271 | 0.159 | 0.333 | 0.285 | 0.010 | 535 (0.043) | 0.0000 |
| truncation, f = 0.5 | 128 | 16,384 | 0.161 | 0.079 | 0.148 | 0.186 | 0.010 | 1,482 (0.118) | 0.0000 |
| truncation, f = 0.9 | 128 | 16,384 | 0.041 | 0.023 | 0.035 | 0.041 | 0.010 | 4,474 (0.358) | 0.0000 |
| truncation, f = 0.99 | 128 | 16,384 | 0.041 | 0.028 | 0.074 | 0.027 | 0.010 | 8,314 (0.665) | 0.0000 |
| truncation, f = 0.999 | 128 | 16,384 | 0.046 | 0.017 | 0.025 | 0.025 | 0.010 | 10,873 (0.869) | 0.0000 |

Diagnostic at the matched count k = 16: the global operator's event-level loss is 0.130 against the local operator's 0.182, and its band error 0.148 against 0.063 — `events_vs_observables` **true**.

Nd II, live bands g r i z J H K; the local control differs from the coarse kernel by 2.1 σ; K*_local = 2, K*_global = None, L* = 0.113; **H1 GREEN, H2 YELLOW**:

| family | archetypes | matrix parameters | max abs dm | mean abs dm | max abs dcolour | SED L1 | m_event | exit lines kept | fallback |
|---|---|---|---|---|---|---|---|---|---|
| local, N_g = 2 | 2 | 4 | 0.069 | 0.019 | 0.047 | 0.013 | 0.189 | all | — |
| local, N_g = 4 | 4 | 16 | 0.063 | 0.019 | 0.035 | 0.013 | 0.172 | all | — |
| local, N_g = 8 | 8 | 64 | 0.023 | 0.011 | 0.032 | 0.014 | 0.169 | all | — |
| local, N_g = 16 | 16 | 256 | 0.012 | 0.006 | 0.016 | 0.013 | 0.153 | all | — |
| local, N_g = 32 | 32 | 1024 | 0.008 | 0.003 | 0.010 | 0.012 | 0.132 | all | — |
| global, rank 1 | 1 | 256 | 0.590 | 0.214 | 0.560 | 0.178 | 0.235 | all | 0.0000 |
| global, rank 2 | 2 | 512 | 0.573 | 0.217 | 0.545 | 0.183 | 0.229 | all | 0.0000 |
| global, rank 4 | 4 | 1,024 | 0.560 | 0.264 | 0.588 | 0.213 | 0.211 | all | 0.0000 |
| global, rank 8 | 8 | 2,048 | 0.294 | 0.157 | 0.342 | 0.100 | 0.175 | all | 0.0000 |
| global, rank 16 | 16 | 4,096 | 0.182 | 0.128 | 0.146 | 0.079 | 0.119 | all | 0.0000 |
| global, rank 32 | 32 | 8,192 | 0.162 | 0.119 | 0.180 | 0.075 | 0.075 | all | 0.0000 |
| truncation, f = 0.1 | 128 | 16,384 | 0.498 | 0.252 | 0.605 | 0.264 | 0.005 | 366 (0.004) | 0.0000 |
| truncation, f = 0.2 | 128 | 16,384 | 0.484 | 0.158 | 0.269 | 0.196 | 0.005 | 697 (0.008) | 0.0000 |
| truncation, f = 0.5 | 128 | 16,384 | 0.176 | 0.050 | 0.089 | 0.096 | 0.005 | 2,027 (0.024) | 0.0000 |
| truncation, f = 0.9 | 128 | 16,384 | 0.022 | 0.009 | 0.023 | 0.019 | 0.005 | 9,742 (0.113) | 0.0000 |
| truncation, f = 0.99 | 128 | 16,384 | 0.003 | 0.001 | 0.004 | 0.011 | 0.005 | 33,822 (0.392) | 0.0000 |
| truncation, f = 0.999 | 128 | 16,384 | 0.016 | 0.008 | 0.015 | 0.011 | 0.005 | 66,045 (0.766) | 0.0000 |

Diagnostic at the matched count k = 2: the global operator's event-level loss is 0.229 against the local operator's 0.189, and its band error 0.573 against 0.069 — `events_vs_observables` **false**.

**Readings.** No gray condition fired on any ion. H1: K*_local = 2, 16, 2
and K*_global = 2, 32, undefined for La II, Ce II, Nd II, so K*_global ≥
K*_local on both decisive ions — **H1 Green**. Nd II is the sharp case:
two contiguous frequency blocks reach 0.069 mag, while a free mixture of
thirty-two archetypes with four times as many free numbers never reaches
the threshold at all (0.162 mag at k = 32). Ce II, the only ion whose
K*_local could have fired Red at k ≤ 4, needed thirty-two archetypes
globally against sixteen locally. H2: L* = 0.358 (Ce II) and 0.113
(Nd II), neither ≤ 0.10 and neither > 0.50 — **H2 Yellow**; La II's
L* = 0.504 is a control and decides nothing, and its grid cannot reach
the threshold at all (its table holds 524 lines, saturated, and the
coarsest truncation on the grid already retains 15 % of them).

**The diagnostic the PI asked for.** At thirty-two archetypes every ion
shows the combination in its strong form: the global operator's
event-level total-variation loss is 0.093, 0.058 and 0.075 against the
local operator's 0.311, 0.138 and 0.132 — it fits the microscopic
redistribution two to four times better — while its band error is 0.034,
0.081 and 0.162 mag against the local operator's 0.004, 0.032 and 0.008.
The rank-1 null is the limit of the same statement: one exit distribution
for every input reproduces the events to 0.70–0.24 total-variation loss
and the light to 0.97–0.59 mag. The preregistered matched-count flag
`events_vs_observables`, evaluated at k = K*_local, is true on Ce II
(k = 16: 0.130 against 0.182 on events, 0.148 against 0.063 mag on the
light) and false on La II and Nd II, where K*_local = 2 is too coarse a
count for the comparison to bite; the flag is reported, and by the PI's
amendment it is not part of the pass/fail logic.

**F69 — On identical resolved Sobolev opacity, identical 128-group exit
tables and the energy-conserving downward macroatom as reference, local
frequency coarse-graining reaches the 0.10 mag transport criterion with
no more archetypal exit distributions than a global non-negative
factorisation needs, and on Nd II with far fewer: two contiguous blocks
against a rank-32 mixture that never reaches it. At thirty-two archetypes
the global operator fits the microscopic redistribution two to four times
better than the local one while reproducing the light two to twenty times
worse. The structure the effective operator exploits is smoothness in
frequency, not a globally low-rank redistribution law, and
microscopic-distribution fidelity is not observable-relevant fidelity.
The exit spectrum is compressible but not by an order of magnitude: the
...
# `make` builds manuscript.pdf and si.pdf (five si_tab_*.tex fragments, also from latex_tables.py) and runs
# check_structure.py on both (fails only on the affiliation / repository-URL todos)
