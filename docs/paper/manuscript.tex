...
lines at $300\kms$. A sweep of 36 conditions across four wavelength windows,
...
Sobolev-intrinsic assumptions, locality is not stressed by our
constant-population models---F1 shows it would cancel at leading order even
if it were---so the residual few-percent Sobolev error most plausibly belongs
to the isolation assumption, i.e.\ to overlap within a Doppler width. The
crowding statistics of \S\ref{sec:crowding} confirm such overlap exists in
these data; quantifying its contribution directly is the first item of
future work.
...
Next: explaining the residual few-percent Sobolev error, whose natural
candidate is line overlap within a Doppler width; frame-consistent comparison
at realistic ejecta velocities ($0.1$--$0.3c$); and extension to scattering
...
committed script; the 27-test suite pins the physics of every module.
...
$z_{\rm res}/ct = 7.7\%$ gave a trough matching $\exp[-\taus(1-z_{\rm res}/ct)]$
to four digits rather than $\exp(-\taus)$. The natural conclusion---which we
drew, and published---was that this is the leading relativistic correction and
...
two differ by a factor $(1-b)$---first order in $\beta$, the same order as
the effect being computed. The time term is not negligible even though the
crossing takes a negligible fraction of $t$, because what matters is the
\emph{gradient}, not the elapsed time.
...
$\tau = D\chi'c/|\mathrm{d}\nu'/\mathrm{d}t|$, and the factors of
$\gamma(1-b)$ cancel down to a single Lorentz factor:
\begin{equation}
  \boxed{\;
...
\begin{equation}
  \frac{\tau_{\rm frozen}}{\taus} = \frac{1-\beta}{\gamma} .
\end{equation}
...
assume a Monte Carlo transport code supplies the time-dependent answer, and
we briefly did. It does not, in the configuration used here. Sweeping a
single line's resonance velocity from $\beta = 0.001$ to $0.34$ and comparing
the measured $\tau_{\rm eff} = -\ln(F/F_{\rm cont})$ against both laws gives
an RMS deviation over $\beta_{\rm res} \gtrsim 0.1$ of $0.024$ from the
frozen law and $0.55$ from the worldline law---a factor $23$, and at
$\beta_{\rm res} = 0.34$ the measurement is $0.613$ against a frozen
prediction of $0.618$ and a worldline prediction of $1.43$. \textsc{sedona}
run with \texttt{transport\_steady\_iterate} iterates the radiation field on
a fixed grid at fixed epoch; its source disables hydrodynamics in that mode
and reads no time-stepping parameters. It is a frozen-snapshot calculation.

This matters for the harness. Cross-code comparison requires matching the
\emph{transport treatment} as well as the thermal-emission convention (F8)
and the spectral normalization (\S\ref{sec:breadth}): a third convention,
and the one with the largest lever arm at high velocity. Like-for-like
against a frozen \textsc{sedona} means running the solver in its frozen
mode. At the shell velocities used for the measurements in this paper
($\beta \le 0.01$) the two laws differ by $\lesssim 2\%$ in $\tau$, so this
choice does not affect any result reported here; at kilonova velocities it
would dominate.

\paragraph{What F3 is, finally.} Not a frame ambiguity (our first reading),
not the leading relativistic correction (our second, published, reading), but
an artifact of integrating a frozen snapshot---a problem the photon does not
...
expansion-opacity binning. It is a fourth member of the family this paper is
about. Its practical consequence for the measurements reported here is that at
our shell velocities ($\beta \le 0.01$) the correct correction is
$5\times10^{-5}$: negligible, and therefore \emph{not} an explanation for the
few-percent Sobolev residual of \S\ref{sec:separation}.
...
Appendix~\ref{app:sobolev} derived $\taus$ to first order in $v/c$. Kilonova
ejecta move at $0.1$--$0.3c$, so the neglected terms are not small, and we
found empirically (Finding F3) that a resonance at $z_{\rm res}/ct = 7.7\%$
produced a trough matching $\exp[-\taus(1-z_{\rm res}/ct)]$ to four digits
rather than $\exp(-\taus)$. We originally recorded that as a frame
\emph{ambiguity} to be avoided by keeping resonances slow. The derivation
below shows it is not an ambiguity: it is the leading relativistic
correction, and it is the nonrelativistic $\taus$ that is wrong.
