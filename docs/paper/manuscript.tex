...
resolved-physics calculations agree to $1$--$4\%$ in every experiment, from a
single artificial line to a realistic forest of 153 calibrated
La\,\textsc{ii} lines from the GSI kilonova atomic database. Against this
validated baseline we measure the expansion-opacity error
$\dsob$ in band-averaged transmitted flux. We find that $\dsob$ decomposes
into (i) a \emph{strength-set floor} arising from the per-resonance
substitution $\tau \to 1-e^{-\tau}$, growing from $\sim 3\%$ for weak-line
forests ($\tau_{\max}=0.5$) to $\sim 40$--$45\%$ for strong-line forests
($\tau_{\max}=5$--$50$), and (ii) a line-width term that grows with the
Doppler width and vanishes as widths become small. Crucially, the floor
survives unchanged down to a Doppler width of $1\kms$---essentially the
thermal width of lanthanum---so no refinement of line widths rehabilitates
the approximation in the strong-line regime that lanthanide-rich kilonova
ejecta occupy. We also report a measured cost curve for resolved transport
...
The resolved pair agree to $3.6\%$ on real data; the expansion-opacity run
overestimates the transmitted band flux by $45\%$. Notably this exceeds the
uniform-ladder error ($+35\%$ at $\tau=0.5$ per line): real forests are
dominated by a few strong lines, precisely where the per-crossing cap is
most wrong.
