...
Correcting the solver accordingly improves its agreement with \textsc{sedona}
from $-1.04\%$ to $-0.06\%$ on the La\,\textsc{ii} forest. The practical
...
\paragraph{Independent confirmation, and a sharper harness.} Switching our
deterministic solver from frozen to worldline transport improves its
agreement with \textsc{sedona} on the 153-line La\,\textsc{ii} forest
monotonically as the treatment improves: $-1.04\%$ (frozen, first order),
$-0.53\%$ (frozen, exact SR), $-0.06\%$ (worldline). \textsc{sedona} performs
genuinely time-dependent Monte Carlo transport, so this is an independent
code agreeing with Eq.~(\ref{eq:taurel}), and it improves the harness's
cross-code agreement (Finding F8) from $\sim1\%$ to below $0.1\%$: the
residual $1\%$ was our frozen approximation, not Monte Carlo noise.
