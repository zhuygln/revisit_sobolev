...
matching $e^{-\taus(1-z_{\rm res}/ct)}$ to four digits. We first read this as
a frame ambiguity between observer- and comoving-frame conventions. It is
not: Appendix~\ref{app:frame} derives the Sobolev optical depth keeping $v/c$
exactly and obtains $\tau_{\rm rel} \simeq \taus(1-v/c)$ at first order, so
the offset is the leading \emph{relativistic correction} and it is the
nonrelativistic $\taus$ that is in error. The comparisons in this paper keep
resonances at $v_{\rm bulk}/c < 1\%$, where the correction is below the Monte
Carlo noise floor; at the $0.1$--$0.3c$ of real ejecta it is a $10$--$30\%$
effect on $\tau$ and cannot be neglected.
...
F3 & The $\mathcal{O}(v_{\rm bulk}/c)$ offset from comoving Sobolev $\tau$ is & \S\ref{sec:verification}, \\
   & not a frame ambiguity but the leading relativistic correction & App.~\ref{app:frame} \\
...
\subsection{The relativistic Sobolev optical depth, and what F3 really was}
\label{app:frame}
...
\paragraph{Kinematics.} For homologous flow the gas at position $\mathbf{r}$
moves with $\boldsymbol\beta = \mathbf{r}/(ct)$. Along a ray toward the
observer write $b \equiv \beta_z = z/(ct)$ for the line-of-sight component and
$\beta = r/(ct)$ for the speed. The exact Doppler relation between the
observer frequency $\nu$ and the comoving frequency $\nu'$ is
\begin{equation}
  \nu' = D\,\nu, \qquad D = \gamma\,(1 - b), \qquad
  \gamma = (1-\beta^2)^{-1/2} .
  \label{eq:doppler}
\end{equation}

\paragraph{Two transformations enter.} First, the opacity. The combination
$\nu\chi_\nu$ is a Lorentz invariant, so the lab-frame opacity is
\begin{equation}
  \chi_{\rm lab} = D\,\chi'_{\rm comoving} ,
\end{equation}
i.e.\ absorption is \emph{weaker} in the lab frame by one factor of $D$.
(Our deterministic solver originally omitted this transformation entirely;
see below for why that omission was nearly harmless.) Second, the sweep rate.
Using $\mathrm{d}b/\mathrm{d}z = 1/(ct)$ and
$\mathrm{d}\gamma/\mathrm{d}b = \gamma^3 b$,
\begin{equation}
  \frac{\mathrm{d}\nu'}{\mathrm{d}z}
  = \frac{\nu}{ct}\Big[\gamma^3 b\,(1-b) - \gamma\Big].
  \label{eq:sweep}
\end{equation}

\paragraph{Collapsing the integral.} As in Appendix~\ref{app:sobolev} the
profile integrates to unity in $\nu'$, so
$\tau = D\,\chi' / |\mathrm{d}\nu'/\mathrm{d}z|$ carries only one
\emph{explicit} $D$. But the resonance condition $\nu' = \nu_0$ fixes
$\nu = \nu_0/D$, which puts a second $D$ into Eq.~(\ref{eq:sweep}). With
$\taus = \sigma_{\rm cl} f n_l \lambda_0 t$ as before,
\begin{equation}
  \boxed{\;
  \frac{\tau_{\rm rel}}{\taus}
  = \frac{D^2}{\big|\gamma^3 b\,(1-b) - \gamma\big|}
  \;}
  \label{eq:taurel}
\end{equation}
which reduces to unity as $b\to0$.

\paragraph{The leading behaviour is one Doppler factor, not two.} It is
tempting---we made this error first---to expect $\tau_{\rm rel} \simeq
\taus(1-b)^2$, one factor from each transformation. Expanding
Eq.~(\ref{eq:taurel}) instead gives $D^2 \simeq 1-2b$ over a denominator
$\simeq 1-b$, so
\begin{equation}
  \tau_{\rm rel} \;\simeq\; \taus\,(1 - b)
  \qquad\text{to first order:}
\end{equation}
the denominator cancels one of the two factors. This explains the empirical
F3 result exactly, and it explains something else: an observer-frame
integration that omits the opacity transformation but also mis-states the
sweep rate---our solver's original mode---makes two errors that cancel at
$\mathcal{O}(b)$ and therefore agrees with the exact treatment to that
order, separating from it only at $\mathcal{O}(b^2)$. The agreement was
fortuitous, not by design.

\paragraph{Consequence, and a caution about sign.} If the correct depth is
$\taus(1-b) < \taus$, then a resolved calculation transmits \emph{more} light
than nonrelativistic Sobolev predicts, so the relativistic term contributes
\emph{negatively} to a residual defined as
$(F_{\rm Sobolev} - F_{\rm resolved})/F_{\rm resolved}$. The residual we
measure in \S\ref{sec:separation} is positive. Removing the $v/c$ term would
therefore \emph{increase} the part left to be explained by line overlap,
rather than absorbing it. We flag this because the opposite expectation is
natural, and because the algebra above is not by itself evidence: the
experiment that settles it sweeps the resonance velocity with both solver
modes, \textsc{sedona}, and each candidate $\tau$ on one axis.

\paragraph{A further relativistic subtlety.} Nonrelativistically the
resonance locus is a plane $z = \mathrm{const}$, which is what makes the
$p$-averaging of Appendix~\ref{app:pavg} elementary. Relativistically
$\gamma$ depends on $r$ rather than $z$, so Eq.~(\ref{eq:doppler}) defines a
curved surface; these are the common-direction and common-point frequency
surfaces of \citet{jeffery1993}, whose relativistic Sobolev method for
homologously expanding atmospheres is the established treatment. Our
controlled experiments keep the emitting core small compared with the
resonance radii, so all core rays are nearly radial and the plane picture
remains a good approximation; a general treatment would need the CD/CP
surfaces explicitly.

%=====================================================================
\vspace{1em}
\noindent\todo{One field outstanding: the journal volume and publication year
of \citet{gsi_atomic}. Its authors, title, arXiv identifier and DOI are
confirmed, but the APS and ADS records refused automated retrieval; check the
publisher page directly. Every other entry has been verified field by field.}
