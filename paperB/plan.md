...
identical physical states, is
    ...
    ε*   versus   R_ij   versus   the energy-conserving downward macroatom reference.
...
- Paper IV's `A2` leg is the energy-packet version at N_g = 32 on the full
  13-ion P1 mixture: within 0.05 mag of the energy-conserving downward
  macroatom (F61). It is a prior for question 1 at the largest N_g, not an answer at
...
- **There is no nuclear-heating or composition-realisation ensemble in this
  ...
  Ye-0.29a pattern); G4 calls that spread σ_comp, reserves the name
  σ_nuclear for an external nuclear-realisation ensemble run through the
  same transport, and says so.
...
| **G4** | negligible against astrophysical uncertainty? | σ_R from R_{N_g*} − macroatom on each composition pattern; σ_comp across the patterns (a composition proxy; σ_nuclear is reserved for an external nuclear ensemble) | σ_R ≪ σ_comp | σ_R comparable to σ_comp |
...
3×10⁵ packets per seed. The reference is the energy-conserving downward
macroatom (`sobolev_dmacro`, Paper IV's R2), on the evaluation seeds
1–3. The operator legs are `sobolev_group` with a kernel built from the
reference's events on separate build seeds 101–103, pooled, at N_g = 2, 4,
8, 16, 32 log-spaced groups with discrete within-group exit tables and
energy rows, and transported on the evaluation seeds; a 128-group matrix
from the evaluation events is the independent fine reference of the
event-level metric. N_g² is the coarse matrix's degrees of freedom, not
the model's complexity; the serialized kernel size is recorded alongside. The scalar legs are `sobolev_tla` at
...
against the coarse matrix size for the three ions with ε* as a line and
the serialized size alongside.
