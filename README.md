...
| F59 | *(superseded — affected by transport bug 38ebf30; do not use quantitatively or scientifically)* **Dead-end handling is in the error budget:** depositing macroatom activations with no radiative exit instead of re-emitting them thermally moves B₂ − R₂ on the lanthanide-rich state from −2.9/+1.3 to −4.9/+0.1 mag (z/K); the thin state moves ≤ 0.3 mag; the thermal balance of Phases 9–10 closes it | [§4.54](docs/results_report.md) |
| F60 | **The 2–5 mag grouped-opacity error of Papers III–IV on saturated lanthanide zones was an off-by-one in the bin legs' opacity inversion (`nu_of_G`), found by multi-shell transport: the same physical state gave different closure answers with different numbers of shell boundaries.** Fixed: the expansion closure is within 0.2 mag of the energy-conserving reference on every published zone and grid (P1 2 d: z −0.19 single zone, −0.16 on 12 shells; K −0.10 / −0.03), the binned closure +0.2 to +0.9 mag the other way; the Sobolev references and the redistribution control never used the inversion and stand | [§4.55](docs/results_report.md) |
| F58 | **Final compositions (Gillanders 2022 Ye-0.21a pattern at X_lan = 0.11; Ye-0.29a rescaled to 2.5×10⁻³) move nothing by more than 0.15 mag; the corrected closure's error moves ≤ 0.05 mag with its own bin width and line cut** (was ~1 mag pre-fix) | [§4.56](docs/results_report.md) |
| F61 | **Gate 2 is Red on every published state and epoch on the fixed transport: the expansion closure is within 0.2 mag of the energy-conserving reference (P1 1–5 d: z −0.14 to −0.19, K −0.06 to −0.14; P2 g +0.11), the binned closure +0.2 to +0.6 mag the other way, the redistribution control ≤ 0.04 mag; the bookkeeping correction R₁ → R₂ is 12–60 times the closure error.** The program's question is answered: the grouped-opacity error does not survive | [§4.56](docs/results_report.md) |
| F62 | **Under complete thermal redistribution the line-binned closure matches the resolved transport to ≤ 0.1 mag on the saturated lanthanide states (Fontes et al.'s result re-found) and the expansion closure sits 0.1–0.3 mag bright; switching only to energy-conserving fluorescence leaves expansion where it was and moves line-binned to +0.3 to +0.8 mag** — coarse-opacity validation depends on the redistribution it was done under | [§4.57](docs/results_report.md) |
| F63 | **Under a radiation-field-driven macroatom (internal upward transitions under an imposed diluted Planck field) the resolved reference brightens by 1.3–1.8 mag in the optical, the expansion closure stays within 0.18 mag of it in every band, and the line-binned closure keeps +0.7 in g while its red excess vanishes and K flips to −0.25** | [§4.58](docs/results_report.md) |
| F64 | **On the Fontes et al. (2020) simplified problem (pure Nd) as a snapshot, complete thermal redistribution gives their ordering — line-binned within 0.0–0.2 mag of resolved Sobolev in colour, expansion 0.2–0.4 mag too blue — and the single switch to energy-conserving fluorescence leaves expansion unchanged while the line-binned colour error opens to 1.3–1.7 mag in r − K, in all three brackets of the inner boundary (mirror, lab-frame and comoving-frame thermalisation)** | [§4.59](docs/results_report.md) |
| F65 | **Over the Fontes (2020) benchmark as a light curve (4–16 d, time-slab transport), the three opacity treatments order the same way under complete redistribution and under explicit fluorescence — resolved lowest, line-binned within 2–4 %, expansion 15–17 % brighter at the peak — and fluorescence changes the closures' colour errors, not their bolometric ones: line-binned 0.13 → 0.51 mag (z, H up to 0.6–0.8 mag off), expansion 0.20 → 0.32; the line-binned leg's 1.2 % trapped packets do not terminate at a threefold cap, R2 Gray by rule** | [§4.60](docs/results_report.md) |
| F66 | **On the xkn secular-ejecta structure with the P1 composition (2–8 d, the xkn diffusion solution as the photospheric boundary, the outer ejecta transported with explicit fluorescence), no statistically resolved bolometric ordering is present (peaks within 2 %, inside the noise; reading Gray by rule) but the two closures produce distinct chromatic biases: expansion a 0.3 mag blueward offset in i − K, line-binned a 0.4 mag redward one, opposite in sign and larger, its interaction count growing twenty-fold as the photosphere recedes; the photosphere re-emits in its own frame (the review's correction moved the closures by ≤ 0.09 mag)** | [§4.61](docs/results_report.md) |
| F67 | **Paper B gate G1 (preregistered, frozen at f9cfbff): on P1 2 d with each ion alone, a group-to-group operator built from the energy-conserving downward macroatom's own events on separate seeds reproduces its transport to within 0.10 mag at N_g = 2 (La II, Nd II) and 16 (Ce II), while the best scalar ε is the coherent limit and misses by 1.1–1.4 mag on the dense ions; the gate reads B1 Green, B2 Green, B3 Yellow → continue. The discrete exit tables, not the matrix, dominate the kernel's size; La II at its P1 partial density is too thin to discriminate** | [§4.62](docs/results_report.md) |
| F68 | **Paper B after G1: under the radiation-field-driven macroatom (imposed W = ½ field, upward transitions; the reference moves by up to 2.1 mag) the 8-group operator rebuilt from that reference's events reproduces it to 0.015–0.046 mag on La II, Ce II and Nd II, while the downward-trained operator reproduces the downward reference it came from: the compression is not a property of the downward table. The exit tables are already the exact aggregate over distinct exit lines (every entry an exact rest frequency, 24 bytes each), sized by how many exit lines the sample discovers (saturating for La and Ce, still rising for Nd), with 90 % of the exit energy in 5–26 % of them; G2 (locality vs rank; the exit lines the light needs) is preregistered, not run** | [§4.63](docs/results_report.md) |
| F69 | **Paper B gate G2 (preregistered, tagged `paperB-g2-prereg`): on identical exit tables and identical opacity, local frequency coarse-graining needs no more archetypal exit distributions than a global non-negative factorisation to reach 0.10 mag — on Nd II two contiguous blocks against a rank-32 mixture that never reaches it — while at 32 archetypes the global operator fits the microscopic redistribution 2–4× better and reproduces the light 2–20× worse. The mechanism is frequency locality, not global low rank, and microscopic fidelity is not observable-relevant fidelity. The exit spectrum compresses to 36 % (Ce II) and 11 % (Nd II) of its distinct lines, not to the 10 % headline: H1 Green, H2 Yellow** | [§4.64](docs/results_report.md) |
