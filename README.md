...
| F67 | **Paper B gate G1 (provisional, Nd II rerun pending): on P1 2 d with each ion alone, a group-to-group operator built from the energy-conserving downward macroatom's own events on separate seeds reproduces its transport to within 0.10 mag at N_g = 2 (La II, Nd II) and 16 (Ce II), while the best scalar ε is the coherent limit and misses by 1.0–1.4 mag on the dense ions; the preregistered gate reads Yellow (one of two readable ions at ≤ 8 groups), the exit tables, not the matrix, dominate the kernel's size** | [§4.62](docs/results_report.md) |
